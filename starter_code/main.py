...
def runtest(mapfile, start, goal, verbose = True):
  ...
  environment = astar_new.Environment(boundary,blocks,goal)
  # Display the environment
  if verbose:
    fig, ax, hb, hs, hg = draw_map(boundary, blocks, start, goal)
  MPAst = astar_new.AStar()
  # MPRRT = RRT.RRTPlanner()
  # Call the motion planner
  t0 = tic()
  path,nodes = MPAst.plan(start,environment,0.5,10)
  # path = MPRRT.plan(start,environment)
  print(nodes)
  toc(t0,"Planning")
  ...
  for i in range(len(path)-1):
    collisioncheck.append(astar_new.isAllowed(boundary,blocks,path[i],path[i+1]))
  if(any(not value for value in collisioncheck)):
    collision = True
  else:
    collision = False
  # print(collision)
